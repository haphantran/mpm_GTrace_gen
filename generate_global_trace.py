...
<body>
    ...
    <script>
        ...
        // Setup - Portrait orientation (narrow width, tall height)
        const width = 500;  // Narrow for 2-column paper format
        const height = 900; // Tall for vertical layout
        ...
        // Calculate level positions - arrange TOP TO BOTTOM (portrait)
        const levels = [...new Set(nodes.map(d => d.level))].sort((a, b) => a - b);
        const levelHeight = height / (levels.length + 1);
        const levelPositions = {{}};
        levels.forEach((level, i) => {{
            levelPositions[level] = (i + 1) * levelHeight;
        }});

        // Force simulation - portrait orientation (vertical)
        const simulation = d3.forceSimulation(nodes)
            .force("link", d3.forceLink(allLinks).id(d => d.id).distance(70))
            .force("charge", d3.forceManyBody().strength(-200))
            .force("y", d3.forceY(d => levelPositions[d.level]).strength(0.5))  // Y axis for levels (vertical)
            .force("x", d3.forceX(width / 2).strength(0.1))  // X axis for spreading (horizontal)
            .force("collision", d3.forceCollide().radius(d => {{
                if (d.type === 'trace') return 40;
                if (d.type === 'trace_link') return 50;
                return 35;
            }}));
